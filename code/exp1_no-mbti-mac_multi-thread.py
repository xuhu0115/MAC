...
def _run_round(inputs, agent, max_tokens=1024, temperature=0.7, response_format="text"):
    question_results = []
    with ThreadPoolExecutor(max_workers=5) as executor:  # 根据API限制调整并发数
        futures = []
        for original_question, prompt in inputs:
            future = executor.submit(agent.generate, prompt, max_tokens, temperature, response_format)
...
class Evaluator:
    ...
    def _evaluate_coherence_llm(self, input_text: str, output_text: str):
        ...
        response = self.multi_response(prompt, max_attempts=5, max_tokens=1024, temperature=0.7, response_format="json_object")
        return response

    def _evaluate_rhythm(self, text: str):
        """评估诗歌韵律"""
        # 使用pypinyin获取拼音
        try:
